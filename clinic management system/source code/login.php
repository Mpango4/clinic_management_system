...
//echo $pass;
  if($_POST['user'] == 'admin'){
    ...
    $sql = "SELECT * FROM professionals WHERE login_id='" .$email . "' and password = '". $pass."'";
    $result = mysqli_query($conn,$sql);
    $row  = mysqli_fetch_array($result);
    //print_r($row);    

    $_SESSION["professional_id"] = $row['professional_id'];
     $_SESSION["id"] = $row['professional_id'];
     $_SESSION["password"] = $row['password'];
     $_SESSION["email"] = $row['login_id'];
