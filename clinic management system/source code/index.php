...
<div class="row col-sm-12"><h3>Welcome <?php echo 'Dr/Nurse. '.$_SESSION['fname']; ?></h3><br><br></div>
<div class="col-xl-3 col-md-6">
<div class="card bg-c-green update-card">
<div class="card-block">
<div class="row align-items-end">
...
<h4 class="text-white">
    <?php
        $sql = "SELECT * FROM appointment WHERE `professional_id`=1 AND appointmentdate=' ".date("Y-m-d")."' and delete_status='0'";
        $qsql = mysqli_query($conn,$sql);
        echo mysqli_num_rows($qsql);
    ?>
</h4>
...
<h4 class="text-white">
<?php
  $sql = "SELECT * FROM appointment WHERE status='Active' AND `professional_id`=1 AND appointmentdate=' ".date("Y-m-d")."' and delete_status='0'" ;
  $qsql = mysqli_query($conn,$sql);
  echo mysqli_num_rows($qsql);
?>
</h4>
...
// Fetch the patient's reminders
$sqlReminders = "SELECT * FROM reminders WHERE patientid='$_SESSION[patientid]' AND status='Active' ORDER BY next_date ASC";
$qsqlReminders = mysqli_query($conn, $sqlReminders);
$rsReminders = mysqli_fetch_array($qsqlReminders);
  ...
  <div class="card row col-lg-12">
      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <ul class="nav nav-tabs md-tabs" role="tablist">
              ...
              <li class="nav-item">
                  <a class="nav-link" data-toggle="tab" href="#profile4" role="tab">Remender</a>
                  <div class="slide"></div>
              </li>
          </ul>
          <!-- Tab panes -->
    ...
    <div class="tab-pane" id="profile4" role="tabpanel">
        <p class="m-0">
            <b>Reminder</b>
            <?php
            if(mysqli_num_rows($qsqlReminders) == 0)
            {
                ?>
                <h3>No Reminder records found.. </h3>
                <?php
            }
            else
            {
                ?>
                <h3>Last Visit taken on - <?php echo $rsReminders['last_date']; ?></h3>
                <h3>Next Visit scheduled on - <?php echo $rsReminders['next_date']; ?></h3>
                <?php
            }
            ?>
        </p>
