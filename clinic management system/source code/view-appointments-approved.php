...
<tbody>
<?php
        $sql ="SELECT * FROM appointment WHERE (status='Approved' ) and  professional_id='$_SESSION[professional_id]' and delete_status='0' and delete_status = '0'";
        ...
        while($rs = mysqli_fetch_array($qsql))
        {
          ...
          if($rs['status'] != "Approved")
          {
            if(!(isset($_SESSION['patientid'])))
            {
              echo "<a href='view-pending-appointment.php?approveid=$rs[appointmentid]&patientid=$rs[patientid]' class='btn btn-xs btn-primary'>Approved</a>";
            }
