...
if(isset($_POST["btn_update"]))
{
    ...
    if($_SESSION['user'] == 'admin'){
      ...
      $q1="UPDATE professionals SET `name`='$fname',`login_id`='$email',`mobile_no`='$contact' where professional_id = '".$_SESSION["id"]."'";
...
<?php
    if($_SESSION['user'] == 'admin'){
      ...
      $que="select * from  professionals where professional_id = '".$_SESSION["id"]."'";
      $query=$conn->query($que);
      while($row=mysqli_fetch_array($query))
      {
        //print_r($row);
...
<div class="col-sm-4">
<input type="email" class="form-control" id="email" name="email" value="<?php echo $login_id; ?>" placeholder="Enter valid e-mail address..." pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" >
<span class="messages"></span>
</div>
</div>
...
<div class="col-sm-4">
<input type="tel" class="form-control" id="contact" name="contact" value="<?php echo $mobile_no;?>" placeholder="Enter valid contact number..." minlength="10" maxlength="10" pattern="^[0][1-9]\d{1-9}$|^[1-9]\d{1-9}$">
<span class="messages"></span>
</div>
